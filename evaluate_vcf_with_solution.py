...
def compare_calls_with_HiFi(all_variants, mapped_hifi, lengths_of_contigs, false_positive_SNPs_file=None, solution_known = False):

    set_of_contig = {"contig_10"} #to select only one contig
    ...
    file_good_variants = open("good_variants.vcf", "w")
    file_good_variants.write("##fileformat=VCFv4.2\n")
    file_good_variants.write("##INFO=<ID=CALLERS,Number=.,Type=String,Description=\"List of callers that called this variant\">\n")
    file_good_variants.write("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\n")

    #list all contigs where deepvariant actually has variants (because of a bug, deepvariant doesn't call variants on some contigs)
    deepvariant_contigs = set()
    for variant_key, variant_info in variants.items():
        contig, pos = variant_key
        callers = [c for c, _ in variant_info.keys() if c != "ref"]
        if "deepvariant_normalized.vcf" in callers:
            deepvariant_contigs.add(contig)

    for variant_key, variant_info in variants.items():

        #skip if the contig is ignored by deepvariant
        if variant_key[0] not in deepvariant_contigs:
            continue
        ...
        ref_base = "B"
        depth_total = 1
        for (c, d), base in variant_info.items():
            if c == "ref":
                depth_total = d
                if contig not in depth_per_contig:
                    depth_per_contig[contig] = 0
                    loci_per_contig[contig] = 0
                depth_per_contig[contig]+= depth_total
                loci_per_contig[contig] += 1
                ref_base = base
                break
        ...
        for caller_and_depth, alt_base in variant_info.items():
            ...
            if caller == "ref" :
                continue
            # if variant_info[caller_and_depth] == "*": #if we don't want to count, indels, uncomment that
            #     continue
            ...
            if variant_key in base_counts:
                ...
                if (alt_base in counts and counts[alt_base] > 0) or (len(alt_base) > len(ref_base) and counts["+"] > 0): #then the variant is validated by hifi
                    ...
                    found_variant = True 
                    if (variant_key, alt_base) not in true_variant_called:
                        true_variant_called.add((variant_key, alt_base))
    ...
    for caller in caller_stats:
        ...
        for contig in contig_stats:
            # Skip contigs where DeepVariant have 0 recall, it is buggy deepvariant contigs
            if caller in contig_stats[contig]:
                if "deepvariant_normalized.vcf" not in contig_stats[contig] or contig_stats[contig]["deepvariant_normalized.vcf"]["recall"] == 0 :
                    # or "snoopy_normalized.vcf" not in contig_stats[contig] or contig_stats[contig]["snoopy_normalized.vcf"]["recall"] == 0 \
                    # or "bcfcalls_normalized.vcf" not in contig_stats[contig] or contig_stats[contig]["bcfcalls_normalized.vcf"]["recall"] == 0:
                    continue
            
                if True or (contig in alt_per_contig and alt_per_contig[contig]/tot_per_contig[contig] >10 ): #average depth of the allele in the contig
    ...
    metacaller_key = "snoopy_normalized.vcf"
    ...
    if metacaller_key in caller_stats and deepvariant_key in caller_stats:
        ...
        for contig in contig_stats:
            if total_number_of_true_variants_per_contig[contig] > 0:
                ...
                cov = min(40, depth_per_contig[contig]/loci_per_contig[contig])
                coverages.append(cov)
                lengths.append(min(lengths_of_contigs.get(contig), 5000))
        ...
        # plt.colorbar(scatter, label="Lengths of contigs")
        plt.tight_layout()
        plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--vcf", required=True, nargs='+', help="Path to the merged vcf file")
    ...
    parser.add_argument("-s", "--solution", required=False, help="Path to the solution genomes file")
    ...
    if not args.solution and not args.mapped_reads:
        print("Error: Either a solution genome file (--solution) or HiFi reads (--mapped_reads) must be provided.")
        sys.exit(1)
    elif args.solution:
    ...
    if reference_genome_available :
            ...
            for line in f:
                if line.startswith(">"):
                    if contig_name is not None:
                        lengths_of_contigs[contig_name] = contig_length
        ...
        compare_calls_with_HiFi(args.vcf, mapped_reads_bam, lengths_of_contigs, solution_known=False)
